mask_detect하는 모델 mask_detector.model이고, 

[[OpenCV 딥러닝] 미리 학습된 파일을 OpenCV DNN 모듈로 딥러닝 실행하기 - cv2.dnn.readNet](https://deep-learning-study.tistory.com/299)

gan 모델을 flask를 사용한 웹에서 colab으로 돌리기

(20210518_web_gan.ipynb)

순서

colab에서 flask-ngrok 설치

```bash
!pip install flask-ngrok
```

[app.py](http://app.py) 가 있는 위치로 이동

```bash
cd /content/drive/MyDrive/Web
```

[app.py](http://app.py) 실행

```bash
!python app.py
```

[app.py](http://app.py) 파일에는 train() 함수와 get_point()를 추가했고, src_upgraded에는 train에서 불러오는 load, layer, network.py가 있다. 
...
TEMPLATE_FOLDER = '/content/drive/MyDrive/Web/templates' # 임의의 경로
STATIC_FOLDER = '/content/drive/MyDrive/Web/static' # 임의의 경로
...
@app.route('/gan_mask_detect_modeling')
def gan_mask_detect():
...
def upload_test():
    ...
    for file in uploaded_files:
        ...
        IDX += 1
    #return render_template('gan_mask_detect_modeling.html')
    return redirect(url_for("gan_mask_detect"))
...
def train():
    ...
    while True:
        ...
        if sess.run(epoch) <= PRETRAIN_EPOCH:
            ...
            if sess.run(epoch) == PRETRAIN_EPOCH:
                saver.save(sess, './src_upgraded/backup/pretrained', write_meta_graph=False)
